...
public class Light_Reflection_Simulator extends PApplet {




float limit_ref_min = 0.5f, limit_ref_max = 2.5f;
...
public void setup()
{
  ...
  ref = 1.3f; // refractive index (changable)
...
// draw lines to show mirrors
  ...
  while (pxL < M22.xx() && pxL >= M21.xx() && count < 2000) { // while the x coordinate fits the screen or reflections are less than 20000
    ...
    if (upar) { // if the point we're dealing with is on the upper mirror (M1)
      ...
      if (count < 300 ) {
        text(con, pxx.get(count)-7, M22.yy() - 37 + ((PApplet.parseInt((con%4)/3))*10) );  // displays the numner of times the ray has reflected yet
        con++;
        } 
      }
    count++;
...
//println(width + "  " + height );
  if (ri) dpx++;
  if (le) dpx--;
  if (pl) { ref += 0.005f; pl = false;}
  if (mi) { ref -= 0.005f; mi = false;}
...
public void keyReleased()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Light_Reflection_Simulator" };
    if (passedArgs != null) {
